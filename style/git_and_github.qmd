...
* indicate the lifecycle stage of the repository (see 'Lifecycle' section below), for example, use `README` badges to indicate statuses like passing checks, [project lifecycle stage](https://www.repostatus.org/) and publication to package indexes (e.g. PyPi, CRAN)
...
* make clear the preferred Git branching strategy (e.g. feature branches off main)
...
It should be clear to users what lifecycle stage each of our repos is at.
This signals how mature the content is, how useable it might be and how much support we can provide.
It can help set expectations and prevent misunderstandings.

Two statuses are particularly important to flag.
When the repository:

1. Is a work in progress or demo, which means that code is not guaranteed to work and features may be missing.
2. Has been archived, which means the content is no longer in use, will not be developed further and that little to no support will be provided.

Codeowners are responsible for considering how best to indicate a repository's lifecycle stage.
Some ideas are to:

* add a note early in the README to explain how developed the work is (perhaps in [a callout](https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax#alerts))
* add a README badge from [repostatus.org](https://www.repostatus.org/), particularly 'WIP' (work in progress) and active (stable but developing)
* reference the lifecycle stage in the repo's description (to edit, click the cog above the 'About' section of your repository's home page), such as '(work in progress)'
* [archive the repo](https://docs.github.com/en/repositories/archiving-a-github-repository/archiving-repositories), a special status that indicates a repo is not maintained or supported, which keeps the repo open but disables interaction

We prefer to archive repositories rather than delete them.
This keeps our work in the open so that we and others can reuse, develop and/or learn from it later.
This also makes it easier to reactivate a repository in future, should the need arise.
