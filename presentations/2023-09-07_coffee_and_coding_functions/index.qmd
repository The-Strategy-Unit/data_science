...
new_rtt <- read.csv("rtt clock starts.csv") |>
  clean_names() |>
  rename(provider_code = organisation_code, count = clock_starts) |>
  mutate(
    snapshot_date = dmy(effective_snapshot_date),
    rtt_yrmon = as.yearmon(snapshot_date),
    rtt_mon = lubridate::month(snapshot_date)
  ) |>
  select(
    -c(x, treatment_function_code, effective_snapshot_date, snapshot_date)
  ) |>
  mutate(count = case_when(count == 0 ~ 1, TRUE ~ count))
```
