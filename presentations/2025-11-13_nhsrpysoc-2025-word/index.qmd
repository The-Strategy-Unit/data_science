...
author: 
  - "[Jacqueline Grout](mailto:jacqueline.grout@nhs.net)"
...
::: {.notes}
* From the website: 'The Strategy Unit is a specialist NHS internal consultancy team. We produce high-quality, multi-disciplinary analytical work – and we help people apply the results. Our proposition is simple: better evidence, better decisions, better outcomes.'
...
## Existing Word template 📑 {auto-animate="true" auto-animate-easing="ease-in-out" background-image="template.png" alt="A blurred, zoomed-out view of pages of a complicated-looking Word document with many chunks of text, tables and page orientations. Much of the text is highlighted in green, yellow and blue."}
...
![](issue.png){alt="A screenshot previewing an issue template. There are checklist items explaining the steps to complete to generate a report. There are links to further information and guidance."}
...
::: {.notes}
* The output is a timestamped folder that contains the populated Word doc, subfolders of the charts and calculated values, and a log file that contains information printed to the console (details things like the scheme name and site selections for the run, as well as warning and error messages if present).
:::

## Insert images with [{officer}](https://davidgohel.github.io/officer/) 👮
...
2. [Find](https://davidgohel.github.io/officer/reference/cursor.html)/[replace](https://davidgohel.github.io/officer/reference/body_add_img.html) with `cursor_reach()` and `body_add_img()`.
...
## Insert values with [{officer}](https://davidgohel.github.io/officer/) 👮

![](insert-value.png){alt="Screenshot of text in a Word document that contains placeholders. Two placeholders are in square brackets and they say 'insert value' and then a number (55 and 56 in this case). The other placeholder is in curly brackets and says 'DOCPROPERTY item_54'."}

:::: {.columns}

::: {.column width="70%"}

1. Add doc-property fields, e.g.<p>`{ DOCPROPERTY item_54 }`.
2. Add [custom doc properties](https://davidgohel.github.io/officer/reference/set_doc_properties.html).
3. Update values programmatically with `set_doc_properties()`.
4. Refresh to insert values to fields.
:::

::: {.column width="30%"}
![](custom-properties.png){alt="Screenshot of the custom properties dialog in Microsoft Word. A number of custom properties are shown with names like 'item_01' and placeholder values like 'Insert Value 01' in square brackets."}
:::
...
## Reflecting on it {.inverse .center}
