...
* indicate the lifecycle stage of the reposiitory (see 'Lifecycle' section below), for example, use `README` badges to indicate statuses like passing checks, [project lifecycle stage](https://www.repostatus.org/) and publication to package indexes (e.g. PyPi, CRAN)
...
1. Is a work in progress or demo, when code may not be guaranteed to work and features are missing.
2. Has been archived, when the content is no longer in use, will not be developed further and when little to no support will be provided.

Codeowners are responsible for considering how best to indicate a repository's lifecycle.
Some ideas are to:
