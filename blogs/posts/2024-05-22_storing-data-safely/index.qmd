...
3. Open (or create) your project's `.Renviron` file, used for [storing environmental variables](https://docs.posit.co/ide/user/ide/guide/environments/r/managing-r.html#renviron). You can do this by running `r usethis::edit_r_environ()` from the R console.
...
5. Save and close the `.Renviron` file then either restart your R session (CTRL + Shift + F10 on Windows machines) or run `r readRenviron(".Renviron")` to make the new variables available in your session.
