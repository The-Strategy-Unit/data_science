...
RStudio has a powerful built-in [Command Palette](https://docs.posit.co/ide/user/ide/guide/ui/command-palette.html), which is a special search box that gives instant access to features and settings without needing to find them in the menus. Many of the tips and tricks we discussed can be found by searching with the Palette.

For example, you may have forgotten how to restart R. If you open the Command Palette and start typing 'restart', you'll see the option 'Restart R Session'. Clicking it will do exactly that. Handily, the Palette also displays the keyboard shortcut (<kbd>Control</kbd> + <kbd>Shift</kbd> + <kbd>F10</kbd>) as a reminder.
...
You can see the shortcuts available in RStudio if you navigate to Help > Keyboard Shortcuts Help, or use the shortcut <kbd>Alt</kbd> + <kbd>Shift</kbd> + <kbd>K</kbd>, which will display a lookup of some of the more common examples. You can go to Help > Modify Keyboard Shortcuts... to search all shortcuts and change them to what you prefer[^terminal].

We discussed a number of handy shortcuts[^cut]. You can:
...
* use Find in Files to search for a particular variable, function or string across all the files in your project, using <kbd>Control</kbd> + <kbd>Shift</kbd> + <kbd>F</kbd>

## Themes

You can change a number of settings to alter the theme, colours and fonts to whatever you desire.

You can [change the default theme](https://docs.posit.co/ide/user/ide/guide/ui/appearance.html) in Tools > Global Options... > Appearance > Editor theme and selecting one from the pre-installed list. It's also possible to upload new themes by clicking the 'Add' button and selecting a theme from your computer. These are stored typically in text files with the extension .rsthemes and can be downloaded from the web, or you can create or tweak one yourself. [The {rsthemes} package](https://www.garrickadenbuie.com/project/rsthemes/) has a number of options and also allows you to switch between themes and automatically switch between light and dark themes depending on the time of day.
...
You can take this further. Click the 'Workspace Panes' button (a square with four more inside it) at the top of the IDE, under the menu options and between the 'Git' and 'Addins' buttons, to see a number of settings. For example, you can select 'Console on the right' to move the console from under the script pane, which you may prefer for maximimsing the vertical space in which code is shown. You could also click Pane Layout... in this menu to be taken to Tools > Global Options... > Pane layout, where you can click 'Add Column' to insert new script panes that allow you to inspect and write multiple files side-by-side.
