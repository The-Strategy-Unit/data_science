...
All of your commits should be in branches, the only changes that are made to the `main` branch would be via pull requests (PR's) that have been reviewed by at minimum one colleague.
...
::: aside
Note that draft PR's are only available on public GitHub repos.
:::
