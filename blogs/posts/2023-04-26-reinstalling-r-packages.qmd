...
to run the script. Generally, sourcing a url is a bad idea - the reason for this is if it's not a link that you control, then someone could update the contents and run arbritary code on your machine. In this case, I'm happy to run this as it's my own gist, but you should be mindful if running it yourself!
