...
-   All code should be committed via pull request (PR) to `main`
...
-   ensure that the issue is as detailed as possible as it may need to be picked up by someone else. Even if only you intend to work on the issue, over time it can be easy to forget things if there isn't enough information
...
-   make a new branch to work on that feature. GitHub offers a button to do this automatically on the issue page, on the right-hand side 
...
For repositories where you are working collaboratively, you must do two things:
...
2.  select a person (or people) to review
...
::: aside
If the PR is later assigned back to you to you should immediately pull changes.
:::

There may be times where you cannot be the assignee on a PR and in those situations you should nominate someone else to be the assignee and in charge of the PR, the same rules as in the paragraph above would then apply.
